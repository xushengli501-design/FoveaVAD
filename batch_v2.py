#!/usr/bin/env python3
"""
PAVAD + Kappa-VAD complete integration batch eval.

PAVAD: SRP×CCP semantic + M3/WAKE/PFC postproc (from pre-extracted cache)
Kappa-VAD: vB (MLLM baseline) + κ¹ (interaction) via HTAG forwards

Integration: combine PAVAD semantic score with Kappa interaction correction
using PAVAD's original gate mechanism:
  final = PAVAD_sem + α · max(0, κ_correction - τ) · (1 - PAVAD_sem)
"""
...
# Select videos: 2 per category + 2 normal
...
results = []
all_pavad = []; all_vb = []; all_k1 = []; all_labels = []
t0 = time.time()
...
for vname in selected:
    ...
    for s in range(0, nf-30, STRIDE):
        ...
        try:
            ...
            k1, vb = compute_kappa(wf)
            pavad_w.append(sem_f[min(s+15, L-1)])
            k1_w.append(k1); vb_w.append(vb); labs.append(ia)
        except: pass
        if len(labs) >= MAX_WIN: break

    if len(labs) < 3: continue
    ya = np.array(labs)
    if ya.sum() == 0 or ya.sum() == len(ya): continue  # need both classes

    pa = np.array(pavad_w); ka = np.array(k1_w); va = np.array(vb_w)
    def zs(x): return (x - x.mean()) / (x.std() + 1e-8)

    # PAVAD original gate: use κ¹ as structural correction
    k1_norm = (ka - ka.min()) / (ka.ptp() + 1e-9)
    tau = np.quantile(k1_norm, 0.80)
    gate = np.maximum(0, k1_norm - tau)
    if gate.max() > 1e-9: gate = gate / gate.max()
    pa_norm = (pa - pa.min()) / (pa.ptp() + 1e-9)
    integrated = pa_norm + 0.10 * gate * (1.0 - pa_norm)

    # Simple combine: zscore
    simple_combine = zs(pa) + 0.5 * zs(ka)

    auc_p = roc_auc_score(ya, pa)
    auc_int = roc_auc_score(ya, integrated)
    auc_sim = roc_auc_score(ya, simple_combine)

    print(f'{vname:<35} {len(ya):>3}w PAVAD={auc_p:.3f} Gate={auc_int:.3f} Simple={auc_sim:.3f} ΔG={auc_int-auc_p:+.3f} ΔS={auc_sim-auc_p:+.3f}', flush=True)
    results.append((vname, auc_p, auc_int, auc_sim, len(ya)))

    all_pavad.extend(pa); all_k1.extend(ka); all_vb.extend(va); all_labels.extend(ya)

# Overall
if len(all_labels) > 1:
    pa = np.array(all_pavad); ka = np.array(all_k1); ya = np.array(all_labels)
    pa_n = (pa-pa.min())/(pa.ptp()+1e-9)
    k1_n = (ka-ka.min())/(ka.ptp()+1e-9)
    tau_a = np.quantile(k1_n, 0.80)
    gate_a = np.maximum(0, k1_n - tau_a)
    if gate_a.max() > 1e-9: gate_a = gate_a / gate_a.max()
    integrated_a = pa_n + 0.10 * gate_a * (1.0 - pa_n)
    simple_a = (pa-pa.mean())/(pa.std()+1e-8) + 0.5*(ka-ka.mean())/(ka.std()+1e-8)

    print(f'\n=== OVERALL ({len(ya)} windows, {len(results)} videos) ===')
    print(f'PAVAD original:     AUC={roc_auc_score(ya, pa):.4f}')
    print(f'PAVAD + κ¹ (gate):  AUC={roc_auc_score(ya, integrated_a):.4f}')
    print(f'PAVAD + κ¹ (simple): AUC={roc_auc_score(ya, simple_a):.4f}')

    n_better = sum(1 for _, p, i, s, _ in results if i > p)
    print(f'Gate improves: {n_better}/{len(results)} videos')
